...
class Map:
    ...
    def is_inbound(self, node):
        ...
        # TODO: your code here:
        #####################################################
        return -self.width / 2 <= node.x <= self.width / 2 and -self.height / 2 <= node.y <= self.height / 2
        #####################################################
    ...
    def is_inside_obstacles(self, node, use_all_obstacles=False):
        ...
        for obstacle in obstacles:
            x_coords, y_coords = zip(*[(n.x, n.y) for n in obstacle])
            if min(x_coords) <= node.x <= max(x_coords) and min(y_coords) <= node.y <= max(y_coords):
                return True
        return False
        #####################################################
    ...
    def step_from_to(self, node0, node1, limit=75):
        ...
        # TODO: your code here
        ############################################################################
        dist = math.sqrt((node1.x - node0.x) ** 2 + (node1.y - node0.y) ** 2)
        if dist < limit:
            return node1
        else:
            ratio = limit / dist
            new_x = node0.x + (node1.x - node0.x) * ratio
            new_y = node0.y + (node1.y - node0.y) * ratio
    ...
    def node_generator(self):
        ...
        while True:
            ...
            new_node = Node((x, y))
            if self.is_inbound(new_node) and not self.is_inside_obstacles(new_node):
                return new_node
        ############################################################################
