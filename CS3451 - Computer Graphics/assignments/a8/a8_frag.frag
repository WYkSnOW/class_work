...
#define DURATION 2.5             /* duration of each fireworks period */
...
vec3 renderStars(vec2 fragPos)
{
    ...
    for(float i = 0.; i < NUM_STAR; i++){
        ...
        /* your implementation starts */
        brightness = 0.0004 * (1.0 + sin(TWO_PI * t * 0.2 + i));
        vec3 starColor = vec3(hash1d(i), hash1d(i + 1.), hash1d(i + 2.)); 
        vec2 starPos = hash2d(i); // normalized position of the star
        fragColor += renderParticle(fragPos, starPos, brightness, starColor);
...
{
    vec3 fragColor = vec3(0.0);
    float emitTime = 1.0; // earlier emission

    if(t < emitTime){
        // increase brightness for the "boss particle"
        float brightness = .004;
        ...
        for(float i = 0.; i < NUM_EMISSION; i++){
            vec2 emitVel = hash2d_polar(i) * .8; // increased speed

            // Modify the brightness and colors to be more vivid and to flicker
            float particleBrightness = 0.004 * (1.0 - emitT / DURATION);
            particleBrightness *= (1.0 - hash1d(t + i) * 0.5); 
            // Randomize the color of each particle for a more festive look
            vec3 particleColor = sin(40. * hash3d(i) * t) * 0.5 + 0.5;

            fragColor += simSingleParticle(fragPos, emitPos, emitVel, emitT, particleBrightness, particleColor);
        }
    }
...
vec3 renderFireworks(vec2 fragPos)
{
    ...
    for(float i = 0.; i < NUM_FIREWORKS; i++){
        float launchTime = i * (DURATION / NUM_FIREWORKS); // staggered launch times
        float relTime = Time - launchTime;
        float t = mod(relTime, DURATION);
        float idx = floor(relTime / DURATION);

        // Random starting positions for each firework
        vec2 launchPos = vec2((hash1d(idx) * 2. - 1.) * iResolution.x / iResolution.y, -0.3);
        vec2 launchVel = vec2(-launchPos.x, hash1d(launchTime + 1.) * 0.5 + 1.2); // faster and higher
        // Bright and varied colors for each firework
        vec3 color = vec3(hash1d(i), hash1d(i + 1.), hash1d(i + 2.));
