...
vec4 shading_normal() 
{
    ...
    vec3 mapped_normal = _normal * 0.5 + 0.5;
    return vec4(mapped_normal, 1.f);
...
vec4 shading_phong(Light light, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 specular = ks * light.Is * spec;

    vec3 phong = ka * light.Ia + diffuse + specular;
...
Light get_spinny_light(Light light) 
{
    ...
    //step 7
    // float red = (sin(iTime) + 1.0) / 2.0;
    // float green = (sin(iTime + 2.09439) + 1.0) / 2.0;
    // float blue = (sin(iTime + 4.18879) + 1.0) / 2.0;
...
void main() 
{
    ...
    Light light2 = Light(vec3(-3, 1, 3),
                     ...
                     vec3(0.4, 0.2, 0.3),
                     vec3(0.4, 0.2, 0.3));
    vec4 color1 = shading_phong(light1, e, p, light1.position, n);
    vec4 color2 = shading_phong(light2, e, p, light2.position, n);
