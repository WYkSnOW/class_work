...
vec3 shadingPhong(Light light, int matId, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 color = sampleDiffuse(matId, p);
    ...
    float diff = max(dot(N, L), 0.0); // Ensure dot product is non-negative
    vec3 diffuse = color * diff * light.Id;
...
vec3 sampleDiffuse(int matId, vec3 p) 
{
    ...
    if(matId == 0) {		
        ...
        /* your implementation starts */
        color = texture(floor_color, uv).rgb * mat_color;
		/* your implementation ends */
    }else if(matId == 1) {
        vec3 normP = normalize(p - spheres[matId - 1].ori);
        float u = 0.5 + atan(normP.z, normP.x) / (2.0 * M_PI);
        float v = 0.5 - asin(normP.y) / M_PI;
        vec2 uv = vec2(u, v);
        color = texture(sun, uv).rgb * mat_color;
    } else if(matId == 2) {
        vec3 normP = normalize(p - spheres[matId - 1].ori);
        float u = 0.5 + atan(normP.z, normP.x) / (2.0 * M_PI);
        float v = 0.5 - asin(normP.y) / M_PI;
...
bool isShadowed(Light light, Hit h) 
{
    ...
    if(h.matId >= 1 && h.matId <= 3) {
        return false;
    }

    bool shadowed = false;                          /* returned boolean to indicate if there is shadow or not */
	vec3 offset_origin = h.p + Epsilon * h.normal;  /* hit intersect */
	vec3 toLight = light.position - offset_origin;  /* vector pointing from the shadow ray origin to the light */
	float t_max = length(toLight);                  /* length of toLight */
	vec3 dir = normalize(toLight);                  /* direction of toLight */
	
    /* your implementation starts */
    Ray shadowRay = Ray(offset_origin, dir);
    ...
    for(int i = 0; i < recursiveDepth; i++) {
        ...
        for(int i = 0; i < recursiveDepth; i++) {
            ...
            if(hit.t > Epsilon) {
                ...
                vec3 reflected_dir = reflect(normalize(recursiveRay.dir), hit.normal);

                vec3 offset_origin = hit.p + Epsilon * hit.normal;

                recursiveRay = Ray(offset_origin, reflected_dir);
                compounded_kr *= materials[hit.matId].kr * 2;
            } else {
                break;
            }
