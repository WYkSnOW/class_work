...
vec4 shading_texture_with_checkerboard() 
{
    ...
    vec2 checkerPattern = fract(scaledUV);
    bool isCheckerBlack = (mod(floor(scaledUV.x) + floor(scaledUV.y), 2.0) == 0.0);
    color = isCheckerBlack ? vec3(0.0) : vec3(1.0);

    //color = vec3(checkerValue);
...
vec4 shading_texture_with_phong(Light light, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 diffuse = kd * light.Id * diff * tex_color;
...
vec3 read_normal_texture() 
{
    ...
    normal = texture(tex_normal, vtx_uv).rgb;
...
vec4 shading_texture_with_normal_mapping()
{
    ...
    perturbed_normal = calc_perturbed_normal(TBN, texture_normal);
